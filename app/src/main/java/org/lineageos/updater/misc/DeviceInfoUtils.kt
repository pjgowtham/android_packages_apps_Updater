...
object DeviceInfoUtils : SettingsLibDeviceInfoUtils() {
    ...
    val isABDevice: Boolean by lazy {
        SystemProperties.getBoolean(PROP_AB_DEVICE, false)
    }
    ...
    private var _isRecoveryUpdateEnabled: Boolean? = null

    @JvmStatic
    var isRecoveryUpdateEnabled: Boolean
        get() = _isRecoveryUpdateEnabled ?: SystemProperties.getBoolean(PROP_UPDATE_RECOVERY, false)
            .also {
                _isRecoveryUpdateEnabled = it
            }
        set(value) {
            _isRecoveryUpdateEnabled = value
