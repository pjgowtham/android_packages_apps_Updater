 ...
 *
 * Use [org.lineageos.updater.repository.UpdaterDbRepository] to interact with data.
 * Schema exports are disabled as history is not formally tracked here.
 */
@Database(entities = [UpdateEntity::class], version = 2, exportSchema = false)
abstract class UpdaterDatabase : RoomDatabase() {
    abstract fun updateDao(): UpdateDao

    fun repository(): UpdaterDbRepository = repositoryInstance ?: synchronized(this) {
        repositoryInstance ?: UpdaterDbRepository(updateDao()).also { repositoryInstance = it }
    }

    @Volatile private var repositoryInstance: UpdaterDbRepository? = null
