...
class NotificationHelper(context: Context) {
    ...
    companion object {
        ...
        private const val NOTIFICATION_ID_NEW_UPDATES = 1
        private const val NOTIFICATION_ID_INSTALL_ERROR = 0
    }
    ...
    fun showNewUpdatesNotification() {
        val intent = PendingIntent.getActivity(
            appContext, 0,
            Intent(appContext, UpdatesActivity::class.java),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
        )
        val notification = Notification.Builder(appContext, CHANNEL_NEW_UPDATES).setAutoCancel(true)
            .setCategory(Notification.CATEGORY_SYSTEM).setVisibility(Notification.VISIBILITY_PUBLIC)
    ...
    fun showUpdateFailedNotification() {
        ...
        val notification =
            Notification.Builder(appContext, CHANNEL_INSTALL_ERROR).setAutoCancel(true)
                ...
                .setStyle(Notification.BigTextStyle().bigText(buildInfo)).setContentText(buildInfo)
                .build()
        notificationManager.notify(NOTIFICATION_ID_INSTALL_ERROR, notification)
    }
}
