...
class UpdateCheckRepository(private val context: Context) {
    ...
    /**
     * Blocking fetch for use outside coroutine contexts (e.g. [UpdatesCheckReceiver]).
     * Returns true if new updates were found and a notification should be shown.
     */
    @Throws(IOException::class)
    fun fetchUpdatesBlocking(): Boolean {
        val jsonFile = Utils.getCachedUpdateList(context)
        val jsonFileTmp = File("${jsonFile.absolutePath}${UUID.randomUUID()}")
        val url = Utils.getServerURL(context)
        Log.d(TAG, "Checking $url")
        ...
        try {
            ...
            conn.inputStream.use { input ->
                jsonFileTmp.outputStream().use { output ->
                    input.copyTo(output)
                }
            }
        } finally {
        ...
        val newUpdatesFound = jsonFile.exists() && Utils.checkForNewUpdates(jsonFile, jsonFileTmp)

        jsonFileTmp.renameTo(jsonFile)
        prefs.edit { putLong(KEY_LAST_UPDATE_CHECK, System.currentTimeMillis()) }

        return newUpdatesFound
    }
