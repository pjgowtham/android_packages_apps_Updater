 ...
 * - 2: Persistent (Verified/Installing/failed install). On restart, restored as [VERIFIED].
 *
...
enum class UpdateStatus(val persistentStatus: Int) {
    ...
    /** True when the verified local package should remain actionable. */
    fun hasVerifiedPackage(): Boolean = when (this) {
        VERIFIED,
        INSTALLATION_FAILED,
