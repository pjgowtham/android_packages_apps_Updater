...
class UpdaterRepository(private val context: Context) {
    ...
    private fun readCache(): List<UpdateInfo> =
    ...
    private suspend fun fetchAndCache(): UpdateResult {
        ...
        return UpdateResult(
            updates = updates.sortedByDescending { it.timestamp },
            isStale = false,
        )
    }

    private suspend fun fetchJson(): String = withContext(Dispatchers.IO) {
    ...
    private fun parseAndFilter(file: File): List<UpdateInfo> =
